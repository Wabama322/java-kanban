...
public class Converter {
    ...
    public static Task fromStringToTask(String value) {
        String[] taskLines = value.split(",", 10);
        ...
        LocalDateTime startTime = LocalDateTime.parse(taskLines[6],formatter);

        if (type.equals("EPIC")) {
            LocalDateTime endTime = LocalDateTime.parse(taskLines[9]);
            return new Epic(name, description, id, status, new ArrayList<>(), startTime, duration, endTime);
        } else if (type.equals("SUBTASK") && taskLines.length >= 8) {
            int epicId = Integer.parseInt(taskLines[7].trim());
